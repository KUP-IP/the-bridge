...
- **Cloud whole-transcript parse** — `MemoryHubCloudParse` does structured-JSON intent extraction over the **entire** transcript (the 4000-char parser cap is now a local-only artifact; frontier large-context one-shots long memos). Reuses the existing `CloudChatTransport` + provider config; the API key stays Keychain-only and is never logged. A durable `.understand/cloud_parse` activity receipt (hash + excerpt, never the full transcript) is written whenever the cloud rung fires, so off-device sends are auditable.
- **Cockpit UX remediation** — full scrollable transcript (was capped at 900 chars), full readable title (was clipped to a 1-line field), an explicit "Transcribing on-device…" state when selecting an un-transcribed memo, human labels (no more `registry_update` / `suppressed`), a commit-value preview ("Will write…"), and a provenance badge.
- **Trust intact:** the chain only changes *who* parses; produced intents feed the unchanged election / processed-gate / guardrails / append-only commit. No new MCP tools (`staticFeatureModuleToolCount` 187). 5-lens adversarial review (0 violations); all 6 actionable findings remediated. test-floor 2602 → **2667**.
- **Deferred to Phase 2:** local-rung map-reduce chunking for long transcripts (frontier covers them now); cost/quota metering; layout redesign; cloud-confidence ↔ guardrail calibration.
...
- **Build 63 (on-device smoke fix)** — memo titles now humanize the real `YYYYMMDD HHMMSS <hexid>` Apple/import filename (e.g. `20260624 162727 760C66A8`); the hex suffix was previously mistaken for a user-given name and shown raw in the cockpit/Inbox. `isDefaultName` now matches the leading timestamp stamp regardless of suffix.
...
- **Data-Source Registry.** A config-driven registry mapping `entity type → Notion data source + property map (bound by PROPERTY ID)`, generalizing the Skills pattern so the same tools work against any Notion. New `Modules/Registry/`: typed exportable config, a generalized per-entity read-through cache (stale-while-revalidate + offline), schema introspection (bind-by-name → ids + type-drift), a central 2 req/s Notion rate limiter, and a generic write path (create-then-update / update / soft-delete). One small MCP surface — **9 `registry_*` tools** (entities · add_entity · introspect · list · get · create · update · delete · possess) serving every entity — plus a Settings → **Data Sources** pane (propose→confirm onboarding). Live-validated end-to-end against real Notion (Skills, Projects, People, Blocks) incl. full CRUD lifecycle.
- **Loopback `/mcp` is never bearer-gated (PKT-810 R5).** Origin split: a direct-loopback request (no Cloudflare `Cf-*` tunnel header) is served token-free — matching the documented "Local clients on this Mac connect with no token" contract — while REMOTE (tunnel) requests stay fully OAuth-gated. **Supersedes the v3.7.x `loopbackStaticBearerFallback` "Retained" note below:** that fallback and `ConnectorAuthContext.localBearer` are **removed** (they gated loopback behind a bearer the OAuth desktop client never sends, dead-ending local clients in a cloud OAuth discovery). Remote bearer/OAuth enforcement is unchanged; tool-level SecurityGate tiers still apply on every call regardless of origin.
