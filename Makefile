...
# → cmd-w2 (Commands data layer) → cmd-w3 (Commands palette: wired
# hot-key → panel → fuzzy-search → W2 body fetch → paste; default-OFF,
# additive-isolated; +21 imported spike + 34 W3, 2026-05-18): runs the
# suite + asserts the green floor (1010) and zero failures. Used by CI so
# a shrunk/disabled suite fails. Floor provenance lives in
