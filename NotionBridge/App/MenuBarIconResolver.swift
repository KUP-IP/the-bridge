...
// resource bundle `NotionBridge_NotionBridge.bundle` WITHOUT a `Contents/` dir
...
//
// This file lives in the NotionBridgeLib target (alongside AppDelegate) so the
...
public enum MenuBarIconResolver {
    ...
    public static func candidateBundlePaths(forMainBundle main: Bundle) -> [String] {
        ...
        if let resourceURL = main.resourceURL {
            ...
            paths.append(
                resourceURL.appendingPathComponent("NotionBridge_NotionBridge.bundle").path
            )
            // SPM library-target resource bundle (sibling).
            paths.append(
                resourceURL.appendingPathComponent("NotionBridge_NotionBridgeLib.bundle").path
            )
