...
// TheBridge · Tests/IntegrationTests
//
...
func runEndToEndTests() async {
    ...
    await test("E2E: Cross-module — file_write then file_read round-trip") {
        ...
        let testContent = "TheBridge integration test: \(Date().ISO8601Format())"
    ...
    await test("E2E: All static feature modules registered with correct tool counts") {
        ...
        try expect(session.count == 5, "SessionModule: expected 5 (tools_search + audit_recent)")
        ...
        // Wave 2 Notion REST: +8 tools (26 → 34).
        try expect(notion.count == 34, "NotionModule: expected 34 (Wave 2 REST)")
        ...
        // fetch_skill, skills_routing_list, 5 skill_* primitives, the 4-tool
        // Runtime Exposure control plane, and skill_materialize_file (#254) = 12.
        try expect(chrome.isEmpty, "ChromeModule removed in v3.7.11 — expected 0 chrome tools")
        try expect(skills.count == 12, "SkillsModule: expected 12 with Runtime Exposure + materialize")
