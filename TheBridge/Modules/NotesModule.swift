...
// entitlement (already granted to TheBridge.app). The per-app Notes
// TCC grant is an operator first-use prompt (System Settings > Privacy &
// Security > Automation > TheBridge → Notes) — no entitlement change.
//
...
public enum NotesModule {
    ...
    /// Automation grant to TheBridge.app itself — one permanent grant,
    ...
    static func failureValue(message: String, code: Int) -> Value {
        ...
        if code == -1743 {
            ...
            obj["guidance"] = .string(
                "The Bridge does not have Automation permission for Notes. "
                + "This is a macOS TCC restriction (first-use operator grant). "
                + "Open System Settings > Privacy & Security > Automation and grant "
                + "The Bridge access to Notes, or open The Bridge's permission panel."
            )
        }
        return .object(obj)
    }
