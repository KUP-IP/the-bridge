...
extension SettingsView {
    ...
    var factoryResetConfirmationMessage: String {
        """
        This will clear: SSE port, stored credentials (Notion workspace tokens and Stripe), onboarding state, and macOS permissions for The Bridge.
    ...
    func performFactoryReset() async -> (message: String, didFail: Bool) {
        ...
        // 3) Remove all saved keychain items for The Bridge.
        ...
        if failures.isEmpty {
            return ("Factory reset complete. Restart The Bridge, then re-grant permissions.", false)
        }
    ...
    func resetTCCPermissions() async -> (message: String, didFail: Bool) {
        ...
        if failures.isEmpty {
            return (
                message: "Permissions reset. Re-grant each permission in System Settings → Privacy & Security, then quit and reopen The Bridge.",
                didFail: false
            )
        }
