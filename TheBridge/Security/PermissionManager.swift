...
//   Fixes: TheBridge invisible in Automation prefs when Chrome was the
...
//   binary identity (TheBridge.app) as applescript_exec, eliminating -1743 errors.
...
public final class PermissionManager {
    ...
    public enum Grant: String, CaseIterable, Identifiable, Sendable {
        ...
        public var settingsHint: String {
            switch self {
            case .accessibility:
                return "System Settings → Privacy & Security → Accessibility → enable The Bridge."
            case .screenRecording:
                return "System Settings → Privacy & Security → Screen Recording → enable The Bridge (relaunch if status does not update)."
            case .fullDiskAccess:
                return "System Settings → Privacy & Security → Full Disk Access → enable The Bridge (relaunch to apply)."
            case .automation:
                return "System Settings → Privacy & Security → Automation → enable each target app under The Bridge."
            case .notifications:
                return "System Settings → Notifications → The Bridge → allow notifications."
            case .contacts:
                return "System Settings → Privacy & Security → Contacts → enable The Bridge."
            case .reminders:
                return "System Settings → Privacy & Security → Reminders → enable The Bridge."
            case .calendar:
                return "System Settings → Privacy & Security → Calendars → enable The Bridge."
            }
        }
    ...
    /// Defines an application that TheBridge may send Apple Events to.
    ...
    /// TheBridge in the Automation preferences pane.
    ...
    /// All known Automation targets. Add new entries here when TheBridge
    ...
    private func notificationSettingsDetail(_ settings: UNNotificationSettings) -> String {
        ...
        case .unknown:
            return "macOS has not resolved the notification prompt for The Bridge yet."
        case .restartRecommended:
    ...
    public func remediation(for grant: Grant) -> String {
        ...
        case .screenRecording:
            return "Enable in Screen Recording. Relaunch The Bridge if status does not update."
        case .fullDiskAccess:
            return "Enable in Full Disk Access. Relaunch The Bridge to ensure new entitlement is observed."
        case .automation:
        ...
        case .notifications:
            if notificationStatus == .unknown {
                return "Click Allow to trigger the macOS Notifications prompt. If macOS still does not resolve the state, The Bridge should open Notifications settings automatically."
            }
            if notificationStatus == .partiallyGranted {
                return "Notifications are authorized, but delivery is muted. Open System Settings > Notifications and enable alerts, Notification Center, sound, or badges for The Bridge."
            }
            return "Open System Settings > Notifications and enable The Bridge if notifications were denied."
        case .contacts:
            if contactsStatus == .unknown {
                return "Click Allow to trigger the macOS Contacts prompt. If macOS does not resolve the state, The Bridge should open Contacts settings automatically."
            }
            return "Open System Settings > Privacy & Security > Contacts and enable The Bridge. Contacts privacy access is separate from Automation access for Contacts.app."
        case .reminders:
            if remindersStatus == .unknown {
                return "Click Allow to trigger the macOS Reminders prompt. If macOS does not resolve the state, open System Settings > Privacy & Security > Reminders and enable The Bridge."
            }
            return "Open System Settings > Privacy & Security > Reminders and enable The Bridge."
        case .calendar:
            if calendarStatus == .unknown {
                return "Click Allow to trigger the macOS Calendar prompt. If macOS does not resolve the state, open System Settings > Privacy & Security > Calendars and enable The Bridge."
            }
            return "Open System Settings > Privacy & Security > Calendars and enable The Bridge."
        }
    }
    ...
    /// to align TCC identity with applescript_exec. Both now use TheBridge.app's
    /// TCC entry, eliminating the identity split where probes authorized /usr/bin/osascript
    /// but execution ran as TheBridge.app (causing runtime -1743 errors).
    ///
