...
public enum CredentialError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .keychainError(let status):
            ...
            if status == -34018 {
                return "This keychain item can’t be deleted from The Bridge. It was likely saved by another app. Remove it in Keychain Access (search the service name), or use that app’s settings."
            }
...
public final class CredentialManager: Sendable {
    ...
    public static func isKeychainItemManagedByThisApp(_ item: [String: Any]) -> Bool {
        guard let expected = defaultKeychainAccessGroupForThisApp() else {
            ...
            // keychain-access-groups in TheBridge.entitlements). The
