...
|---|---|---|---|
| 1 Non-overlap | gap — needs latch/test | unknown | unknown (parallel-by-design) |
...
1. **Non-overlap (capability 1) — BLOCKING.** Prove the provider deterministically refuses a second concurrent invocation of the same routine (→ `NOT_STARTED_OVERLAP`), OR implement+test a deterministic fail-closed latch (cycle reads a single-writer flag in Notion/The Bridge and aborts before any FOCUS write). PRD §6 step 1 hard-fails the cycle if overlap "cannot be ruled out." *No doc resolves this for any provider — it is the single most important test.*
