...
**Recommended: Claude Code (Routines)** — §8.5A tie-breaker (already-connected, strongest documented isolation, simplest native scheduling). Full 9×3 matrix with per-cell evidence status in `config/PROVIDER_CAPABILITY_MATRIX.md`. **Selection is NOT final.** Capability #1 (routine-level non-overlap): doc research (2026-06-23) **confirmed** Claude Code Routines documents no non-overlap guard → built + unit-tested a §8.5A-permitted controller-side fail-closed latch (`overlap_gate`/`overlap_verify`, **T99 PASS**); the **live end-to-end overlap experiment is still pending** (needs a real routine — see [`evidence/INTEGRATION_NON_OVERLAP.md`](./evidence/INTEGRATION_NON_OVERLAP.md)). It remains the single most important live integration test. Blocking cells that must be integration-verified before scheduling: non-overlap (#1), native completion notification w/ brief link (#8), machine-callable pause/fail-closed latch (#7), plus scheduled-run execution-reference, failure-derivation, and Bridge-MCP reachability. Session export (#9) may use the §8.18 fallback (non-blocking). **Capabilities #7/#8 researched 2026-06-24:** #7 (pause) is UI-only → resolved by the §8.5A controller kill-switch latch (T57/T58 PASS); #8 (completion notification) has **no native notification + no PRD-permitted workaround** → it is the deciding provider fork. Full synthesis + recommendation: [`config/PROVIDER_DECISION_PROPOSAL.md`](./config/PROVIDER_DECISION_PROPOSAL.md).
