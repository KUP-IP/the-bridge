...
- 2026-08-21 · A1 canonical PACKETS preflight · `3767 → 3776` (+9): one canonical persisted/cache identity with `session` as a request alias, complete classification of the 34-property live execution schema, exact binding/type/option/relation-target drift, and byte-preserving read-only preflight coverage. Measured 3776 passed, 0 failed on codex/a1-integrity-preflight-v2.
- 2026-08-21 · bounded THREAD Messages M1 recovery · `3776 → 3780` (+4 combined): explicit service is validated before side effects, recovered local evidence must match that service, duplicate Intent retries fail closed, and an exact existing Result can complete operator-review recovery without resending or duplicating the journal. Individually measured 3771 passed, 0 failed on codex/thread-m1-reactivation-v2.
- 2026-08-21 · U2 governed Node tests · `3780 → 3788` (+8 combined): owner-bound trusted node:test imports with fixed permissions, bounded output/timeout, and post-run identity revalidation. Individually measured 3775 passed, 0 failed on codex/u2-governed-node-test-v2.
- 2026-08-22 · U2 macOS physical-path integration · `3788 → 3789` (+1): resolve the special `/tmp → /private/tmp` alias with POSIX `realpath` before constructing Node filesystem permissions, preventing valid ESM tests from being denied in symlink-rooted worktrees.
- 2026-08-23 · Messages delivery-correlation numeric affinity · `3789 → 3790` (+1): coerce the text-bound preparation timestamp to `REAL` before comparing it with the computed Messages timestamp, preventing a verified local outbound row from being filtered out by SQLite storage-class ordering. Measured 3790 passed, 0 failed on codex/fix-messages-delivery-correlation.
- 2026-08-31 · issue #198 inherit live inbound service · `3790 → 3844` (+54): `messages_send` inherits latest inbound iMessage/SMS or fails closed; explicit SMS on live iMessage/RCS refuses silent fallback; read tools expose `message.service`. +5 hermetic on this branch; remaining already green on main `707b8f4` vs the stale 3790 floor. Measured 3844 passed, 0 failed on feat/issue-198-inherit-service.
- 2026-09-02 · #234 PACKETS alias load canonicalize · `3844 → 3848` (+4): `RegistryConfig.canonicalizePacketAliases` on load/save collapses PACKETS-shaped `session`+`packet` rows to one `packet` and rewrites disk; genuine Sessions entities stay. Rebased onto main after #200. Measured 3848 passed, 0 failed on fix/234-packet-alias.
- 2026-09-02 · #184 awaiting_approval before MCP timeout · `3848 → 3852` (+4): `SecurityGate` returns `awaiting_approval` before the MCP client times out; retry after Allow reaches the handler. Measured 3852 passed, 0 failed on feat/issue-184-awaiting-approval after rebase onto main (post-#239).
- 2026-09-02 · #189 inbound /health counters · `3852 → 3854` (+2): expose `/mcp` inbound count on `/health` for Notion connect flake diagnosis. Measured 3854 passed, 0 failed on feat/issue-189-inbound-health after rebase onto main (post-#201).
- 2026-09-02 · #232–#235 registry Wave 1 · `3854 → 3859` (+5): binder follows property id across rename; writes keyed by decoded id; cache revalidates `lastEditedTime`; Notion URL-or-UUID including `app.notion.com`. Measured 3859 passed, 0 failed on wave1-registry-234.
- 2026-09-02 · Notion REST Wave 2 · `3859 → 3890` (+31): view query/delete, page trash/restore, search `in_trash`, comment PATCH/DELETE, async tasks, templates XOR children, meeting-notes query, opt-in multi_part upload (#225–#231, #236–#237). Measured 3890 passed, 0 failed on wave2-notion-rest.
- 2026-09-03 · hotfix #238 Safari Cmd+V · `3920 → 3927` (+7): Safari/WebKit prefers paste; leftover ⌃⌥⇧ HID key-up before Cmd+V (Command held); Command-held UCKeyTranslate for V; pasteboard restore-on-throw; gh_pr_review drops `comment`; keyboard_type copy drops Input Monitoring claim; Contacts `.limited` sufficient (live runner is PermissionsModuleTests — `runPermissionManagerTests` is skipped because `checkAll()` hangs). CI measured 3926 passed / 0 failed on `1f18ed4` before moving the Contacts test; floor 3927 after that move.
- 2026-09-04 · #249 RCS/unknown SMS operator override · `3927 → 3935` (+8): `allowSmsDespiteLiveService` unlocks explicit SMS only when live inbound is RCS/unknown. Omit still inherit-or-refuse; flag does not unlock iMessage↔SMS mismatch; RCS stays out of the send enum. +8 net-new MessagesModuleTests. CI measured 3935 passed, 0 failed on `cursor/rcs-sms-operator-override-394d` (run 33825894053).
- 2026-09-04 · #251 Cmd+V prior-clipboard race · `3935 → 3942` (+7): `prePasteDelay` 60ms / `postPasteDelay` 120ms (≥ Handy 60/60; 2× after-delay because Bridge restores). `waitUntilPublished` polls `changeCount`+string before Cmd+V. Restore stays in `defer` after the consume window (including on throw). +7 CommandCursorInsertTests (delay constants, recording-sleep restore-after-perform, restore-on-throw+postDelay, empty prior, fake delayed consumer, race documentation, publish poll). CI measured 3942 passed, 0 failed on `cursor/fix-251-cmdv-prior-clipboard-f9ab` (run 33897067337).
- 2026-09-04 · Runtime Exposure optional Deprecation Date + orphan purge · `3942 → 3947` (+5): missing SKILLS `Deprecation Date` warns instead of `schema_missing` hard-fail; `skills_exposure_purge_orphans` drops named local+published orphans after a lifecycle decision; `outreach-dispatch` HOLD is refused by generic sweep. +4 SkillExposureAuthorityTests +1 SkillsModuleTests. Floor is 3942 + net-new after rebase onto #252. Pre-rebase isolated CI measured 3940 (run 33895348693); CI must re-measure.
- 2026-09-04 · #256 Time Keepr–style screen filenames · `3947 → 3967` (+20): `b-{ISOWeek}.{ISOWeekday}-{NN}.{ext}` shared daily sequence (png+mp4), Mac local calendar, leftover `nb-screen-*` ignored. Cleanup keeps same-day `b-*` (no 1h wipe) and deletes prior local days. +20 hermetic ScreenArtifactNamingTests after rebase onto main post-#253. Floor is main 3947 + net-new tests.
- 2026-09-04 · #258 notify-default + Always Allow everywhere · `3967 → 3973` (+6): `neverAutoApprove` is not an execution floor; every Confirm notification offers Always Allow (sticky Notify); `standing_orders_delete` registered Request; new `ToolRegistration` default tier is Notify. +6 SecurityGateUXTests. CI must re-measure.
- 2026-09-04 · #258 live-verify Confirm delivery fallback · `3973 → 3983` (+10): menu-bar Confirm + Security ATTENTION for in-flight Request (remote and local); request notifications populate PKT-553 userInfo; content extension DefaultContentHidden=false. +10 SecurityGateUXTests. CI must re-measure.
- 2026-09-04 · #260 live-fail Confirm body on status-item click · `3983 → 3989` (+6): sticky Confirm panel (Deny/Allow/Always Allow) on status-item click and pending Request; badge not cleared until resolved; SECURITY_APPROVAL default/dismiss present the body instead of Deny; Always Allow still sticky Notify. +6 SecurityGateUXTests. CI must re-measure.
- 2026-09-06 · calendar free/busy v0 (Isaiah GO) · `3989 → 4013` (+24): read-only `calendar_free_busy`; occupancy SSOT = FOCUS EventKit `A33CAC6E-9D15-44F4-BC35-54F204F4DA39` only (Meetings / Google Meetings freeBusy out of scope per ISAIAH live probe). Half-open `[start, end)`; fail-closed missing FOCUS / permission / bad date / non-FOCUS calendarId. +23 CalendarModuleTests + 1 access-denied nested. CI must re-measure.
- 2026-09-06 · #263 Request-tier awaiting_approval · `4013 → 4017` (+4): first Request call returns pending immediately (no 25s UN wait / LSUIElement NSAlert auto-allow); late Always Allow is a retry ticket; clean-prefs remote `standing_orders_delete` does not archive until Allow. +4 SecurityGateUXTests. CI measured 4017 passed, 0 failed on `4d91254` (run 34060644427).
- 2026-09-07 · #264 notify stickies only on explicit Always Allow · `4017 → 4027` (+10): compact `SECURITY_APPROVAL` first action is Allow (not Always Allow); Confirm NSPanel assigns no default button; `persistNotifySticky` logs tool / module / source. +10 SecurityGateUXTests. Isolated #264 measure was `4013 → 4023` on CI run 34061106853 (SHA 35d2737); stacked onto #263 as 4017 + 10.
- 2026-09-07 · #262 assertive Confirm + Time Sensitive · `4027 → 4034` (+7): sticky panel auto-presents on escalate; Always Allow visual primary (not Return default); Time Sensitive authorization + interruption; Confirm keeps `.regular` so LSUIElement cannot hide it; dual-notify / Focus docs. +7 ConfirmPresentationUXTests stacked on #267. CI must re-measure.
- 2026-09-07 · #262 LIVE FAIL rewrite floor + race · `4034 → 4038` (+4): pending-surface auto-front extras on `be574b51`. CI run 34138318943 measured 4038 total / 4037 passed; `second escalate re-asserts presented` lost a MainActor race (observer `Task` hop overwrote `statusItemClick`). Observer is `MainActor.assumeIsolated`; test asserts click + re-front on one hop. Floor is the measured green count.
- 2026-09-08 · #262/#264 LIVE fail beyond PR #269 · `4038 → 4047` (+9): LSUIElement force-front plan (`.regular` before create) + `persistNotifySticky` refuses implicit-foreground / pending-escalate / default-button. +9 ConfirmLiveFailContractTests. Linux host cannot compile macOS 26 — CI must re-measure.
- 2026-09-12 · #262/#264 LIVE fail beyond PR #271 · `4047 → 4051` (+4): WindowServer yield-before-create (same-turn `.regular`+create still `windows=0`) + UN `ALWAYS_ALLOW` is `presentBody` / refuses `.notificationAlwaysAllow` (PR #271 classified live banner as explicit persist). +4 ConfirmLiveFailContractTests (probe skip-yield, probe run, present drives run, handleConfirmNotificationAction). Linux host cannot compile macOS 26 — CI must re-measure.
- 2026-09-13 · #254 skill files catalog + materialize (merged onto #273) · `4051 → 4071` (+20): Skills seed binds `files` / `googleDriveFile` / `manager`; `fetch_skill` emits `files[]` (empty-honest) + optional `assetRoot`; `skill_materialize_file` copies Notion-hosted attachments into `skill-files/<uuid>/` for `file_read`. Notion is not binary SSOT. +17 FetchSkillNotionFilesTests +2 RegistryConfigTests +1 SkillsModuleTests. `staticFeatureModuleToolCount` 239 → 240. Stacked on main `74c61503` (#273 floor 4051). Linux host cannot compile macOS 26; CI must re-measure.
- 2026-09-14 · #276 fetch_skill Files & media catalog identity · `4071 → 4079` (+8): Live Notion GET `files` items have name + signed URL, not a top-level `id`. Catalog takes `notionFileId` from `file_upload.id` or the S3 attachment UUID. Stale empty flatten overlays a fresh getPage catalog when online; body cache persists the catalog. Kind/host allowlist unchanged. +7 FetchSkillNotionFilesTests +1 SkillBodyCacheTests. CI run 34860177190 measured passed=4079 (floor was wrongly 4080). Linux host cannot compile macOS 26.
- 2026-09-20 · #284 Jobs gut + local-model connection removal (parent #282) · `4095 → 4010` (−85): DROP product Jobs (UI/models/persistence/MCP/`NBJobRunner`) and Ollama connection (tools/Advanced/Memory). Platform routines replace scheduling — no Jobs replacement UI. Memory stays tool-first peek. Deleted JobsModuleTests (45) + SchedulerResilienceTests (33) + CronHumanizerTests (3) + OllamaModuleTests (6) + job_*/ollama annotation/settings/seeder cases; +8 ProductSurfaceGutTests. Floor lowered with recorded reason. Linux host cannot compile macOS 26; CI must re-measure.
- 2026-09-21 · #284 CI-measured floor · `4010 → 4000` (−10): CI run 35525944536 on `cfb31f4c` measured passed=4000, 0 failed. Source `await test(` net remains −85 vs main 4095. Stamp the measured green count (same convention as #276). Linux host cannot compile macOS 26.
- 2026-09-21 · #298 messages_send notify-default 1:1 · `4000 → 4004` (+4): ordinary 1:1 plain text is catalog Notify; groups / attachments / SMS-override stay Request. +4 MessagesSendApprovalPolicyTests. CI run 35636531485 measured passed=4004, 0 failed on 818ba83.
- 2026-09-22 · #303 / epic #301 protocol discriminator · `4004 → 4020` (+16): inherit live 1:1 thread/contact service (inbound, else unambiguous chat.guid / service_name) or fail closed; 1:1 chatIdentifier uses the same bind (no AppleScript first-match remap). +16 MessagesProtocolDiscriminatorTests. Linux host cannot compile macOS 26; CI must re-measure.
- 2026-09-23 · SMS Continuity-dead honesty · `4030 → 4034` (+4): `messages_send` fails closed (`sent=false`, claimable error) when a correlated SMS row has error≠0, is_sent=0, is_delivered=0, and empty destination_caller_id. Dest-present SMS stays #302. +4 MessagesSendStatusHonestyTests. Stacked on main `d1ada9bf` after #307; prior 4031 measure is void. Linux host cannot compile macOS 26; CI must re-measure.
- 2026-09-26 · #312 url_open · `4034 → 4046` (+12): first-class notify-tier `url_open` (http/https/notion, not worktree-gated) + `shell_exec open <http(s)>` `worktree_target_unresolved` remedy names `url_open` / `applescript_exec open location`. +7 SystemModuleTests +2 WorktreeOwnershipTests +3 URLOpenPolicyTests. `staticFeatureModuleToolCount` 225 → 226. Linux host cannot compile macOS 26; CI must re-measure.
