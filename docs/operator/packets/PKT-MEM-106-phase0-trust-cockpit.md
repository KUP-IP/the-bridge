...
> **Contract status.** Phase 0 is decision-complete for execution; do not open new product/design surveys inside 0a/0b/0c unless implementation evidence conflicts with a trust invariant. Residual A–E choices are handled through PKT-MEM-112 after Phase 0.
...
- **Trust invariants are sacred** (PKT-MEM-105): exactly one primary lane auto-executes; suppressed lanes queue to review with reason `secondary intent suppressed`; protected registry text fields (`brief`, `objective`, `summary`, `description`) are **append-only, never overwritten**; a memo is marked processed **only when no pending review remains for that memo**; agent memory stores the **full transcript**. Activity receipts are privacy-limited and must never store full transcripts. No Phase 0 path may weaken these for automation rate.
- **Review semantics are explicit:** suppressed lanes are `pending` review entries, not a batch backlog; Process and Inbox render the same underlying entries. `voice_memo_commit`, `voice_memo_review_resolve`, and mark-handled paths may mark processed only after the last pending sibling clears; dismiss removes a review entry but does not write or mark processed by itself.
...
- Memory Hub files live under `~/Library/Application Support/TheBridge/memory-hub/` only (`activity.jsonl`, `providers.json`, `registry-cache/<entity>.json`, `plan-snapshots/<memoId>.json`) — separate from voice-memo transcripts; never UserDefaults for evidence.
...
- Do not run M1/M5/M8 as pass/fail live evidence until Slice 0a + 0b land (identity + cockpit/activity receipts). Any pre-0c run is REVIEW/PARTIAL-scope only unless the operator explicitly accepts it; Phase-0 closeout evidence waits for 0c. Live runs are single-memo only — never call `voice_memo_process mode:batch` or batch the backlog.
...
Phase 0 is DONE when, on `feat/memory-hub-voice-curator`: all three slices' Definition-of-Done checklists are green; `make test` passes with `FLOOR=` raised by net-new tests (`≥2303` → the per-slice measured green count, set after re-measuring — the validation plan's projected totals are NOT inherited as hard targets) with dated provenance; the trust invariants above hold under the Slice-0a tests; the cockpit renders the three zones with stable AX IDs and per-intent commit; `activity.jsonl`, `providers.json`, `registry-cache/<entity>.json`, and `plan-snapshots/<memoId>.json` persist at their locked paths; and the operator runs the live re-test order **M1 → M5 → M8** against `make install`-built `The Bridge.app`, producing a markdown grade table with activity-receipt references (per the live suite). Each slice is independently reviewable and shippable; **0a lands first** because it unblocks live M5/M8.

### Output Destination

Implementation lands in this repo on `feat/memory-hub-voice-curator`: source, tests, AX harness, tool metadata, and `scripts/test-floor-gate.sh` provenance live in their natural code paths, with one independently reviewable commit/sub-PR per slice. Live closeout writes the grade artifact to `docs/operator/live-evidence/PKT-MEM-113-M1-M5-M8.md` using activity receipt references from `~/Library/Application Support/TheBridge/memory-hub/activity.jsonl`. Do not create a separate batch/backlog artifact; live evidence is one memo at a time.
...
- Append-only protected fields: `registry_update` writes to all four protected fields (`brief`, `objective`, `summary`, `description`) append and never overwrite.
...
- [ ] **Timeouts/failures.** Heuristic immediate; local enhancement **8s** soft timeout; manual cloud enhancement **20s** timeout. On timeout or cloud failure, keep the latest valid heuristic/local plan and record timeout/failure status in activity. Cloud failure does **not** queue a review item.
- [ ] **Versioned plan snapshots + diff badges.** Preview/enhancement results stored as versioned plan snapshots; UI badges added / changed / demoted-superseded intent fields before commit. **Decision 2 — snapshot storage:** per-memo JSON at `~/Library/Application Support/TheBridge/memory-hub/plan-snapshots/<memoId>.json` (inspectable, prunable). Retention keeps the **heuristic, latest enhanced, and committed** snapshot per memo; intermediate enhanced drafts pruned after a newer enhanced/committed snapshot exists. Pruning runs on snapshot write plus launch/wake sweep.
...
- [ ] **Registry per-field diff apply UX (Decision 4).** Non-protected property updates are committed only from an explicit before/after **per-field diff** with selectable non-protected updates (the diff model itself lands here in 0c, not in 0a). Protected fields (`brief`, `objective`, `summary`, `description`) stay **append-only** and are never offered as overwritable diffs. Diff display shows a human-readable old/new summary with expandable raw JSON. All selected fields validate before any write; any validation failure writes nothing and leaves the intent uncommitted + review-visible. Ambiguous mappings, stale cache targets, or missing selected rows force manual commit.
- [ ] **Processing Keychain provider status/save/delete.** Processing tab shows provider configured/missing status; keys stored in **Keychain only**; supports save/update and delete per provider. **Decision 3 — provider fields:** generic provider slot with **API key + base URL + model name + enabled toggle**, enabling one **OpenAI-compatible** manual enhancement path. Non-secret provider config persists to `~/Library/Application Support/TheBridge/memory-hub/providers.json`; API key lives only in Keychain. Base URL defaults to `https://api.openai.com/v1`; model name is blank and operator-required. Save validates syntax only; network/model validation happens only during manual cloud enhancement. No provider admin / billing / model catalog / usage dashboard.
...
- Timeouts/failures: local 8s soft / cloud 20s fallback to latest valid plan; timeout/failure status recorded in activity; cloud failure does not queue review.
- Snapshots: per-memo JSON at `plan-snapshots/<memoId>.json`; retention keeps heuristic + latest-enhanced + committed; prune on write + launch/wake sweep; diff badges for added/changed/demoted.
- Registry diff (0c): non-protected per-field selectable apply (before/after computed, only chosen fields written after all selected fields validate); protected fields never overwritable; validation failure leaves intent uncommitted/review-visible.
...
- Provider keychain/config: save/status/delete for the OpenAI-compatible slot (API key + base URL + model + enabled); non-secret config in `memory-hub/providers.json`; base URL default only (`https://api.openai.com/v1`), model required; syntax validation on save, network/model validation only on manual enhance; never plaintext surfaced.
- Agent tab: soft forget calls `memory_forget`; pin toggle sets/clears pinned state; no text editing or hard delete path exists.
- Notion tab: open + refresh are present; backfill is dry-run preview first, then selected apply; no full row CRUD path exists.
- Activity corruption: corrupt `activity.jsonl` lines skipped on load, original file preserved, repair activity recorded.

> **Floor bump (0c):** raise `FLOOR=` to the post-0c measured green with a dated provenance comment, e.g. `# PKT-MEM-106 0c preview+guardrails+tabs (2026-06-25): +N PreviewSnapshot/Threshold/DuplicateForce/RegistryDiff/Provider/ActivityRepair/Notification tests; progressive preview + 8s/20s timeouts, per-memo plan-snapshots/<memoId>.json (heuristic/latest/committed), lane thresholds, duplicate force reason enum {new_context,correction,operator_confirmed,live_test}+note, per-field registry diff (protected append-only, summary+raw JSON, validate all selected before write), OpenAI-compatible provider slot (key+baseURL+model+enabled, providers.json non-secret config, syntax-on-save/network-on-enhance), corrupt activity-line repair, precise active-Process notification suppression, Agent forget/pin, Notion refresh/dry-run backfill. Measured green <0b> + N. Phase 0 final floor.` (`+N` measured at merge.)
...
5. **No new Settings section** — all UI lands on existing Memory tabs; match `BridgeTokens` and existing Memory tab patterns. Memory Hub files live only under `~/Library/Application Support/TheBridge/memory-hub/` (`activity.jsonl`, `providers.json`, `registry-cache/<entity>.json`, `plan-snapshots/<memoId>.json`).
...
7. **Live re-test order (Decision 5):** after 0a + 0b land for minimum identity/cockpit proof, and after 0c for Phase-0 closeout evidence, run **M1 → M5 → M8** per `docs/operator/MEMORY-HUB-LIVE-MULTI-INTENT-SUITE.md` — simple trust path (M1) before registry-heavy (M5/M8). Single-memo mode only; never call `voice_memo_process mode:batch` or batch the backlog. Live evidence = activity-log receipt references + a PASS/PARTIAL/FAIL markdown grade table. **Any forced duplicate commit performed during the live suite uses force reason `live_test`** so that audit/cleanup can filter test-originated receipts from genuine operator forces in the shared `activity.jsonl`.
...
- [ ] `activity.jsonl`, `providers.json`, `registry-cache/<entity>.json`, and `plan-snapshots/<memoId>.json` persist at their locked Memory Hub paths with correct retention.
- [ ] Live re-test **M1 → M5 → M8** on `make install`-built `The Bridge.app` writes `docs/operator/live-evidence/PKT-MEM-113-M1-M5-M8.md` with one table row per case: case, build, memo id, grade, receipt refs, cleanup status, notes; session cleanup performed (reminders_delete, registry revert, `memory_forget`, `voice_memo_review_dismiss`, curator mode → Auto). Forced live duplicate commits used reason `live_test`.
